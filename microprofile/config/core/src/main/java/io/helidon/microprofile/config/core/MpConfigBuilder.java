...
import static io.helidon.microprofile.config.core.MpMetaConfig.MetaConfigSource;
...
class MpConfigBuilder implements Builder<MpConfigBuilder, Config>, ConfigBuilder {
    ...
    public Config build() {
        ...
        List<MpConfigFilter> targetFilters = HelidonServiceLoader.create(ServiceLoader.load(MpConfigFilter.class))
                .asList();
        // as we have moved `MpConfigFilter` to MP only, the encryption filter service no longer implements it.
        targetFilters.add(SeFilterWrapper.wrap(new EncryptionFilterService()));
