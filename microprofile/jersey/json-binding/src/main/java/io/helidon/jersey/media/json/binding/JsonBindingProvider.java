...
public class JsonBindingProvider implements MessageBodyReader<Object>, MessageBodyWriter<Object> {
    ...
    public Object readFrom(Class<Object> type,
        ...
        return jsonBinding.deserialize(entityStream, GenericType.create(genericType));
    }
    ...
    public void writeTo(Object object,
        ...
        jsonBinding.serialize(entityStream, object, GenericType.create(genericType));
    }
    ...
    private boolean supportsType(Type type) {
        ...
        boolean registeredConverter = jsonBinding.prototype().serializers().stream()
                .anyMatch(serializer -> serializer.type().equals(genericType))
                && jsonBinding.prototype().deserializers().stream()
                        .anyMatch(deserializer -> deserializer.type().equals(genericType));
        if (registeredConverter) {
            return true;
        }

        boolean genericFactory = jsonBinding.prototype().bindingFactories().stream()
        ...
        if (type instanceof ParameterizedType parameterizedType) {
            ...
            return !Map.class.isAssignableFrom(rawType)
                    || jsonBinding.prototype().serializers().stream()
        ...
        if (type instanceof GenericArrayType genericArrayType) {
            return supports(genericArrayType.getGenericComponentType());
        }
