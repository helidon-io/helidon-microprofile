...
class FailEmitter extends PlainEmitter {
    ...
    void validate(Object item) {
        super.validate(item);
        if (getOverflowStrategy() == OnOverflow.Strategy.FAIL && !hasRequests()) {
            this.error(new IllegalStateException("Backpressure error, downstream didn't request enough data for another send."));
        }
    }
}
