...
Connecting streams to Kafka with Reactive Messaging is easy to do. There is a
standard Kafka client behind the scenes, all the [producer][producer] and
[consumer][consumer] configs can be propagated through messaging config.
...
```xml [pom.xml]
<dependency>
  <groupId>io.helidon.microprofile.messaging.connectors.kafka</groupId>
  <artifactId>helidon-microprofile-messaging-connectors-kafka</artifactId>
</dependency>
```
...
Sends nacked messages to error topic, [DLQ][dlq] is well known pattern for
dealing with unprocessed messages.
...
[helidon-messaging]: https://github.com/helidon-io/helidon-examples/tree/dev-27.x/examples/messaging
