...
`io.helidon.microprofile.config.core.MpConfigSources` class:
...
property `io.helidon.microprofile.config.core.meta-config`, or environment variable
...
<dependency>
  ...
  <artifactId>helidon-microprofile-config-hocon</artifactId>
</dependency>
```
...
implementing `io.helidon.microprofile.config.core.spi.MpMetaConfigProvider` SPI and registering
it as a service (Using `META-INF/services/${class-name}` file when using
classpath, or using the `provides` statement in `module-info.java` when using
module path).

The interface `io.helidon.microprofile.config.core.spi.MpMetaConfigProvider` requires
implementation of the following methods:
