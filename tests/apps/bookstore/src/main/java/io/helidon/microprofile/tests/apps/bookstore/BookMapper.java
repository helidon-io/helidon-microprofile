...
package io.helidon.microprofile.tests.apps.bookstore;
...
public class BookMapper {
    ...
    private static Book updateBook(Book book, JsonObject jo) {
        ...
        if (jo.containsKey(DESCRIPTION)) {
            book.setDescription(jo.getString(DESCRIPTION));
        }
        if (jo.containsKey(SUMMARY)) {
            book.setSummary(jo.getString(SUMMARY));
        }
        if (jo.containsKey(GENRE)) {
            book.setGenre(jo.getString(GENRE));
        }
        if (jo.containsKey(CATEGORY)) {
            book.setCategory(jo.getString(CATEGORY));
        }
        if (jo.containsKey(PUBLISHER)) {
            book.setPublisher(jo.getString(PUBLISHER));
        }
        if (jo.containsKey(COPYRIGHT)) {
            book.setCopyright(jo.getString(COPYRIGHT));
        }
        if (jo.containsKey(AUTHORS)) {
            book.setAuthors(AuthorMapper.decode(jo.getJsonArray(AUTHORS)));
        }
        return book;
    }
}
