...
public class CoordinatorDeployer {
        ...
        containerConfig.addConfigBuilderConsumer(configBuilder -> {
            ...
            configBuilder.withSources(MpConfigSources.create(ConfigSources.create(is, MediaTypes.APPLICATION_X_YAML)),
                                      MpConfigSources.create(Map.of(
                                              ...
                                              "server.port",
                                              String.valueOf(clientPort.get()),
                                              "server.worker-count",
                                              "16",
                                              "server.sockets.0.name",
                                              COORDINATOR_ROUTING_NAME,
                                              // Force coordinator to use random port first time with 0
                                              // reuse port second time(TckRecoveryTests does redeploy)
                                              "server.sockets.0.port",
                                              String.valueOf(coordinatorPort.get()),
                                              "server.sockets.0.worker-count",
                                              "16",
                                              "server.sockets.0.host",
                                              "localhost",
                                              "helidon.lra.coordinator.db.connection.url",
                                              "jdbc:h2:file:./target/lra-coordinator",
                                              "helidon.lra.coordinator.recovery-interval",
                                              "100",
                                              "helidon.lra.coordinator.timeout",
                                              "3000"
                                      )));
        });
    ...
    public void beforeUndeploy(@Priority(Interceptor.Priority.PLATFORM_AFTER) @Observes BeforeUnDeploy event,
                               Container container) {
